...
from contract_parser.models import VendorType
...
SERVICE_CANONICAL: dict[str, str] = {
    ...
    "ground": "ground",  # weak; combined with vendor
    # UPS
...
def normalize_percent(text: str) -> dict[str, Any]:
    ...
    if m:
        ...
        return {"unit": "percent", "value": val, "raw": s, "confidence": 0.9}

    # Plain number with % implied in column header context — caller may pass "10" alone; low confidence
...
def normalize_weight_range(text: str) -> dict[str, Any]:
    ...
    if m:
        ...
        conf = 0.85 if lo <= hi else 0.3
        return {"min": lo, "max": hi, "unit": unit, "raw": s, "confidence": conf}

    sm = _SINGLE_WEIGHT_RE.match(s)
    if sm:
        v = float(sm.group("val"))
        unit = _normalize_weight_unit(sm.group("unit"))
        return {"min": v, "max": v, "unit": unit, "raw": s, "confidence": 0.6}
